...
public class CircularGraphView extends View {
    ...
    public CircularGraphView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //圆环宽度
        mRingWidth = a.getFloat(R.styleable.CircularGraphView_ring_width, 360);
        //最里面的圆的颜色(白色)
        mMinCircleColor = a.getColor(R.styleable.CircularGraphView_circle_color, context.getResources().getColor(R.color.button_two_dimension));
        //圆环的默认颜色(圆环占据的是里面的圆的空间)
        mRingNormalColor = a.getColor(R.styleable.CircularGraphView_ring_normal_color, context.getResources().getColor(R.color.button_two_dimension));
        a.recycle();
        //默认圆环画笔
...
//        canvas.drawText("0点", mViewCenterX - 220, mViewCenterY, dialWordsPaint);
        ...
        //画彩色圆环
        drawColorRing(canvas, mPaint1, 360-sweepAngle1,
                sweepAngle1, Color.parseColor("#6F6F6F"));
...
//        ringNormalPaint.setColor(mRingNormalColor);//圆环默认颜色为灰色
        ringNormalPaint.setColor(getResources().getColor(R.color.color_8DD678));
        canvas.drawArc(mRectF, 270, 90-sweepAngle1, false, ringNormalPaint);
    }
...
//           ringColorPaint.setShader(new SweepGradient(mViewCenterX, mViewCenterX, color, null));
     ...
     * @param
     */
    public void setAnnularData(float sweepAngle1) {
//        , float sweepAngle2,
